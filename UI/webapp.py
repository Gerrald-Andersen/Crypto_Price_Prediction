...
def fetch_volume_data():
    ...
    if "total_volumes" not in data:
        st.error("❌ 'total_volumes' not found in API response. The API may be rate-limited or temporarily unavailable.")
...
lstm_model = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'lib', 'lstm_model.keras'))
scaler_pipeline = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'lib', 'scaler_pipeline.pkl'))
try:
...
try:
    with open(scaler_pipeline, "rb") as f:
